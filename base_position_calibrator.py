...
class BasePositionCalibrator:
    ...
    def __init__(
            ...
            camera: Camera,
            tag: ViewableObject,
            tag_gps_angle_threshold=math.radians(3)
    ):
        self._camera = camera
        self._tag = tag
        self._tag_gps_angle_threshold = tag_gps_angle_threshold

        self._object_motion_analyzer = ObjectMotionAnalyzer(
            self._camera,
            self._tag,
    ...
    def get_object_motion_analyzer(self) -> ObjectMotionAnalyzer:
        return self._object_motion_analyzer

    def get_object_stats_processor(self) -> ObjectsStatsProcessor:
    ...
    def _get_points_where_error_circle_intersections_intersect_each_other(self) -> MultiPoint:
        points = []
    ...
    def get_all_error_circle_intersections(self):
        if self._error_circle_intersections is None:
            self._error_circle_intersections = [
                cc.get_error_circle_intersection()
                for cc in self._get_all_circumcircle_objects()]
